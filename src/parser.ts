...
*            | DefnStmt
*            | Expr
* IfStmt    := _ 'm[áa]' _ expr=Expr _ stmt=Stmt elsebranch={ _ 'n[oó]' _  stmt=Stmt}?
* BlockStmt := _ '{' blk=Stmt* '}'
...
export type Stmt = Stmt_1 | Stmt_2 | Stmt_3 | Stmt_4 | Stmt_5;
export type Stmt_1 = IfStmt;
export type Stmt_2 = BlockStmt;
export type Stmt_3 = AssgnStmt;
export type Stmt_4 = DefnStmt;
export type Stmt_5 = Expr;
...
export class Parser {
    ...
    matchStmt($$dpth : number, cr? : ContextRecorder) : Nullable<Stmt> {
        return this.choice<Stmt>([
            ...
            () => { return this.matchStmt_5($$dpth + 1, cr) },
        ]);
    }
    ...
    matchStmt_5($$dpth : number, cr? : ContextRecorder) : Nullable<Stmt_5> {
        ...
        return this.runner<IfStmt>($$dpth,
            (log) => {
                ...
                if(true
                    ...
                    && this.match_($$dpth + 1, cr) != null
                    && (stmt = this.matchStmt($$dpth + 1, cr)) != null
    ...
    matchBlockStmt($$dpth : number, cr? : ContextRecorder) : Nullable<BlockStmt> {
        return this.runner<BlockStmt>($$dpth,
            (log) => {
                if(log)
                    log('BlockStmt');
                let blk : Nullable<Stmt[]>;
                let res : Nullable<BlockStmt> = null;
                if(true
                    && this.match_($$dpth + 1, cr) != null
                    && this.regexAccept(String.raw`{`, $$dpth+1, cr) != null
                    && (blk = this.loop<Stmt>(()=> this.matchStmt($$dpth + 1, cr), true)) != null
                    && this.regexAccept(String.raw`}`, $$dpth+1, cr) != null
                )
